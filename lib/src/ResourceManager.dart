...
class ResourceManager {
  ...
  static final Logger LOGGER = LoggerFactory.getLogger("ResourceManager");
  ...
  void loadIndex() {
    ...
    Stream st = new Stream(assets["game/monkey/MONKEY1.000"]);
    while (!st.eof()) {
      String name = st.readString(4);
      int len = st.read32BE();
      LOGGER.debug("$name, len=$len");
      if (name == "RNAM") {
        loadRoomNames(st);
      } else if (name == "MAXS"){
        /* skip this section */
  ...
  void loadRoomNames(Stream st) {
    LOGGER.debug("Loading room names");
    while (true) {
      var no = st.read();
      if (no == 0) {
        break;
      ...
      if (tmp.indexOf(0) >= 0) {
        //tmp = tmp.sublist(0, tmp.indexOf(0));
        tmp = new List<int>();
        for (var i = 0; i < tmp.indexOf(0); i++) {
          tmp.add(tmp[i]);
        }
      }
      var name = new String.fromCharCodes(tmp);
      rooms[no] = new Room(no, name);
      LOGGER.debug("$name #$no");
    }
  }
  ...
  Stream loadResource(String type, int roomIdx, int offset) {
    LOGGER.debug("Loading resource of type $type in room #$roomIdx at offset $offset");
    Room room = rooms[roomIdx];
    loadRoomsOffsets(room);
    Stream data = new Stream(assets["game/monkey/MONKEY1.00${room.fileId}"]);
