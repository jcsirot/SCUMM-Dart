...
class ScummVM {
  ...
  static final Logger LOGGER = LoggerFactory.getLogger("ScummVM");
  ...
  ScummVM(Element canvasElement) {
    charsets = new Map<int, Charset>();
    threads = new List<Thread>();
    //timer = new Timer();
    interpreter = new Interpreter();
    actors = new Map<int, Actor>();
    vars = new List<int>(800);
    arrays = new Map<int, List<int>>();
  ...
  void redrawBackground() {
    ...
    /* Only dirty parts should be redrawn */
    gfx.redrawBGStrip(currentRoom, 0, 40);
    objs.forEach((obj) {
      gfx.drawObject(obj);
    });
    /* Clearing the object queue makes objects vanished not work for the moment */
    //objs.clear();
  }
  ...
  void drawActors() {
    ...
    // sort actors
    actors.values.where((Actor a) => a.costume != null).forEach((Actor a) {
      gfx.drawCostume(a);
    });
  }
